...
class PiFlowBaseModel(nn.Module):
    """Base model for Pi-Flow training with policy heads."""
    def __init__(self, args, device):
        super().__init__()
        self._initialize_models(args, device)
    ...
    def _initialize_models(self, args, device):
        """Initialize models with WanPiflowWrapper for generator."""
        self.real_model_name = getattr(args, "real_name", "Wan2.1-T2V-1.3B")
        self.fake_model_name = getattr(args, "fake_name", "Wan2.1-T2V-1.3B")

        # Use WanPiflowWrapper for generator (supports policy heads)
        self.generator = WanPiflowWrapper(**getattr(args, "model_kwargs", {}), is_causal=True)
        self.generator.model.requires_grad_(True)
...
class PiFlowSelfForcingModel(PiFlowBaseModel):
    """Self-forcing model for Pi-Flow training with policy-based ODE integration."""
    def __init__(self, args, device):
        super().__init__(args, device)
        self.denoising_loss_func = get_denoising_loss(args.denoising_loss_type)()
    ...
    def _run_generator(
        ...
        """Run generator with backward simulation and collect piflow data."""
        assert getattr(self.args, "backward_simulation", True), "Backward simulation needs to be enabled"
        if initial_latent is not None:
            conditional_dict["initial_latent"] = initial_latent
        if self.args.i2v:
        ...
        max_num_frames = self.num_training_frames - 1 if self.args.independent_first_frame else self.num_training_frames
        default_min = 20 if self.args.independent_first_frame else 21
        min_num_frames = min(default_min, max_num_frames)
        min_num_frames = (min_num_frames // self.num_frame_per_block) * self.num_frame_per_block
        min_num_frames = max(min_num_frames, self.num_frame_per_block)
        assert max_num_frames % self.num_frame_per_block == 0
        max_num_blocks = max_num_frames // self.num_frame_per_block
        min_num_blocks = min_num_frames // self.num_frame_per_block
        num_generated_blocks = torch.randint(min_num_blocks, max_num_blocks + 1, (1,), device=self.device)
        ...
        noise_shape[1] = num_generated_frames
        input_noise = torch.randn(noise_shape, device=self.device, dtype=self.dtype)

        pred_image_or_video, denoised_timestep_from, denoised_timestep_to, piflow_data = self._consistency_backward_simulation(
            noise=input_noise,
            **conditional_dict,
        )
        ...
        pred_image_or_video_last_21 = pred_image_or_video_last_21.to(self.dtype)
        return pred_image_or_video_last_21, gradient_mask, denoised_timestep_from, denoised_timestep_to, piflow_data
    ...
    def _consistency_backward_simulation(
        ...
        **conditional_dict: dict
        ...
        """Run backward simulation with pi-flow pipeline."""
        if self.inference_pipeline is None:
            self._initialize_inference_pipeline()
    ...
    def _initialize_inference_pipeline(self):
        """Initialize PiFlowTrainingPipeline."""
        from pipeline import PiFlowTrainingPipeline
        
        self.inference_pipeline = PiFlowTrainingPipeline(
            denoising_step_list=self.denoising_step_list,
            scheduler=self.scheduler,
            generator=self.generator,
            num_frame_per_block=self.num_frame_per_block,
            ...
            num_max_frames=self.num_training_frames,
            context_noise=self.args.context_noise,
            policy_type=getattr(self.args, 'policy_type', ''),
            sampling_type=getattr(self.args, 'sampling_type', 'dmd'),
            integration_nfe=getattr(self.args, 'integration_nfe', 128),
            x0_pred_type=getattr(self.args, 'x0_pred_type', 'dmd'),
        )
